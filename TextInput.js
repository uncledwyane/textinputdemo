...
var TextInput = function(parentEl) {
    ...
    this.getCurrTextInfo = function() {
        ...
        const {color, fontSize, fontStyle} = this.currTextElStyleList
        const content = currTextEl.innerText

        // 上顶点高度
        let leftTopPointY = parseInt(y - (currTextEl.offsetHeight / 2))
        let textShape = {
            leftTop: [x, leftTopPointY].toString(),
            rightBottom: [x + parseInt(currTextEl.offsetWidth), leftTopPointY + parseInt(currTextEl.offsetHeight)].toString()
        }
        let textInfo = {
        ...
        console.log('getCurrTextInfo，textInfo: ', textInfo)
    }
}
